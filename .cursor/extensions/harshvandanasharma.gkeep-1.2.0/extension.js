...
function activate(context) {
  ...
  const ignoreFilePath = path.normalize(
  ...
  async function createGitkeepFile(filePath) {
    ...
    await vscode.workspace.fs.writeFile(
      workspaceFolder.uri.with({
        path: path.join(normalizedPath, ".gitkeep"),
      }),
      new Uint8Array()
    );
  }
  ...
  function generateAndCheckDirectories(uri) {
    ...
    if (
      normalizedUri !== workspaceFolder.uri.fsPath &&
      ignoreList.ignores(
        normalizedUri.replace(workspaceFolder.uri.fsPath, "").slice(1)
      )
    ) {
      return;
    }
    ...
    vscode.workspace.fs
      ...
      .then((directoryContents) => {
        if (directoryContents.length === 0) {
          createGitkeepFile(normalizedUri); // Pass the normalized path
        } else {
          directoryContents.forEach(([name, fileType]) => {
            if (fileType === vscode.FileType.Directory) {
              generateAndCheckDirectories(
                uri.with({ path: path.join(normalizedUri, name) }) // Use normalized path
              );
            }
          });
        }
